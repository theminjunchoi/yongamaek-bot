...
좌석맵은 회차당 약 36KB(gzip)라, 열려 있는 58개 회차를 매분 전수 조회하면 요청량이 현행의 15\~20배가 됩니다. CGV 서버가 못 버텨서가 아니라 **WAF에 찍혀 러너 IP가 차단되면 봇 전체가 죽기 때문에** 그렇게 하지 않습니다. 대신:
...
결과적으로 요청은 분당 약 20건(현행의 4\~5배)이고, Actions 러너에서 실측한 사이클 소요는 60초 중 **35초**입니다.
